...
export class UserAsyncService {
  ...
  users = [{id:1, name:"avraham",age:"40",phone:"05468977",gender: 1,smoking:false},
  ...
  updateUserData(user_data) {    
    for (let idx = 0 ; idx < this.users.length ;idx++) {
        if (user_data.id == this.users[idx].id) {
            this.users[idx] = user_data;                    
            break;
        }
    }
